 ...
 * -------------------------------------------------------
 */
class MemberRespository extends BaseRespository{
	...
	protected function fieldsExtra($fields){
		...
		foreach ($fields as $k => $filed){
			if (in_array($filed, [ 'balance', 'purchases' ])  ){
				$fields[$k] = 'b.'.$filed;
			}else{
	...
	protected function joinsExtra(){
		return [
				'left join '.$this->table_prefix.'member_account as b on '.$this->alias.'.user_id = b.user_id'
		];
	}
