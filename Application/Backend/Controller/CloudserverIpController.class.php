...
use Common\Respository\CloudserverIpBusinessRespository;
 ...
 * -------------------------------------------------------
 */
class CloudserverIpController extends BackendController{
	...
	public function _initialize(){
		parent::_initialize();
		$this->model = new \Backend\Model\CloudserverIpModel();
		$this->respository = new CloudserverIpBusinessRespository($this->model);
	}
...
// 		}else {
// 			$msg = '录入业务信息成功';
// 			$url = U('Backend/CloudserverIp/index', [  ], false);
// 			if (IS_AJAX) {
...
// 	dump($res);die;
	...
	public function erase(){
		...
		$id = $request['id'];
		//删除业务操作
		$res = $this->respository->eraseCloudserverIpBusiness($id);
...
// 		dump($entity);die;
	...
	public function sync(){
		...
		$res = $this->respository->CloudserverIpBusinessSynchronizing($id);
// 		dump($this->model);die;
		//返回结果
		if ($res === false) {
			$msg = $this->model->getError();
	...
	public function syncBatch(){
		...
		$flag = $request['flag'];
		//批量同步虚拟主机操作
		$res = $this->respository->syncBatch(GiantAPIParams::PTYPE_CLOUD_SERVER, $flag,  'cloudserver.ip');
