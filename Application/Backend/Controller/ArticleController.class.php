...
use Common\Respository\ArticleRespository;
...
class ArticleController extends BackendController
{
	...
	public function _initialize(){
		parent::_initialize();
		$this->model = new \Backend\Model\ArticleModel();
		$this->respository = new ArticleRespository($this->model);
	}
	...
	public function create(){
		...
		$this->assign([
			'type' => $this->model->searchable['type']['data'],
	...
	public function recording(){
		...
		if ($res === false) {
			...
			$msg = '添加成功';
			$url = U('Backend/Article/index', [  ], false);
			if (IS_AJAX) {
...
// 			dump($res);die;
	...
	public function disable(){
		...
		$data['state'] = 0;
		//更新操作
		$res = M('article')->where([ 'id' => [ 'eq', $id ] ])->setField($data);
