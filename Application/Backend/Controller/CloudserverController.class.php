...
use Common\Respository\CloudserverBusinessRespository;
 ...
 * -------------------------------------------------------
 */
class CloudserverController extends BackendController{
	...
	public function _initialize(){
		parent::_initialize();
		$this->model = new \Backend\Model\CloudserverModel();
		$this->respository = new CloudserverBusinessRespository($this->model);
	}
...
// 	dump($res);die;
	...
	public function erase(){
		...
		$id = $request['id'];
		//删除业务操作
		$res = $this->respository->eraseCloudserverBusiness($id);
...
// 		dump($entity);die;
	...
	public function sync(){
		...
		$res = $this->respository->CloudserverBusinessSynchronizing($id);
// 		dump($this->model);die;
		//返回结果
		if ($res === false) {
			$msg = $this->model->getError();
	...
	public function syncBatch(){
		...
		$flag = $request['flag'];
		//批量同步虚拟主机操作
		$res = $this->respository->syncBatch(GiantAPIParams::PTYPE_CLOUD_SERVER, $flag,  'cloudserver.server');
