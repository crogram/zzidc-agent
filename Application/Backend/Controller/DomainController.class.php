...
use Common\Respository\DomainRespository;
...
class DomainController extends BackendController{
	...
	public function _initialize(){
		parent::_initialize();
		$this->model = new \Backend\Model\DomainModel();
		$this->respository = new DomainRespository($this->model);
	}
...
// 	}
	...
	public function erase(){
		...
		$id = $request['id'];
		//删除业务操作
		$res = $this->respository->eraseDomainBusiness($id);
	...
	public function details() {
		...
		//获取业务的产品配置相关信息
		$contact = (new \Common\Model\DomainContactModel())->firstOrfail($id, 'domain_business_id');
...
// 		dump($contact);die;
	...
	public function sync(){
		...
		$id = $request['id'];
		//同步域名操作
		//因为域名有四个子类型，所以这里不需要再传入ptype参数了，
		//同步方法内部进行了处理，会找到相应的ptype
		$res = $this->respository->DomainBusinessSynchronizing($id);
// 		dump($this->model);die;
		//返回结果
		if ($res === false) {
			$msg = $this->model->getError();
	...
	public function syncBatch(){
		...
		$flag = $request['flag'];
		//批量同步域名操作
		$res = $this->respository->syncBatch(GiantAPIParams::PTYPE_DOMAIN, $flag, 'getallbid');
