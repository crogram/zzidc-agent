...
use Common\Respository\LinksRespository;
...
class LinksController extends BackendController
{
	...
	public function _initialize(){
		parent::_initialize();
		$this->model = new \Backend\Model\LinksModel();
		$this->respository = new LinksRespository($this->model);
	}
	...
	public function recording(){
		...
		if ($res === false) {
			...
			$msg = '添加成功';
			$url = U('Backend/Links/index', [  ], false);
			if (IS_AJAX) {
...
// 			dump($res);die;
		//返回结果
		if ($res === false) {
			$msg = $this->model->getError().'更新友情链接失败，请重试！';
			if (IS_AJAX) {
				$this->error( $msg, '',true);
			}else {
				$this->error($msg);
	...
	public function disable(){
		...
		$data['state'] = 0;
		//更新操作
		$res = M('links')->where([ 'id' => [ 'eq', $id ] ])->setField($data);
	
		//返回结果
		if ($res === false) {
			$msg = $this->model->getError().'，不显示友情链接失败，请重试！';
			if (IS_AJAX) {
				$this->error( $msg, '',true);
			}else {
				$this->error($msg);
	...
	public function enable(){
		...
		//返回结果
		if ($res === false) {
			$msg = $this->model->getError().'，显示友情链接失败，请重试！';
			if (IS_AJAX) {
				$this->error( $msg, '',true);
			}else {
				$this->error($msg);
