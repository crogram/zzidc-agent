 ...
 * -------------------------------------------------------
 */
class CloudspaceModel extends BackendModel{
...
// 		$fill_data['product_type_id'] = D('Common/ProductType')->getAll('id as k,type_name as v');
	 ...
	 * ----------------------------------------------
	 * | 获取完整的一条云空间记录信息
	 * | @时间: 2016年11月5日 下午2:15:15
	 * | @author: duanbin
	 * | @param: variable
	 * | @return: type
	 * ----------------------------------------------
	 */
	public function getCloudspaceInfo($id, $attribute = "a.id"){
		$where[$attribute] = [ 'eq', $id ];
		$res = $this->alias('a')->field('a.*,b.product_name,c.login_name')
		->join([
				'left join '.C('DB_PREFIX').'product as b on a.product_id = b.id',
				'left join '.C('DB_PREFIX').'member as c on a.user_id = c.user_id',
		])->where($where)->find();
		return $res;
	}
	...
	/**
	 * ----------------------------------------------
	 * | 根据域名和cloudspace_id获取相应的站点信息
	 * | @时间: 2016年11月5日 下午2:16:23
	 * | @author: duanbin
	 * | @param: variable
	 * | @return: type
	 * ----------------------------------------------
	 */
	public function getSiteInfoByDomainAndId($doainame, $id){
		$where['site_name'] = [ 'eq', $doainame ];
		$where['cloudspace_id'] = [ 'eq', $id ];
		return M('cloudspace_site')->where($where)->find();
	}
	
	/**
	 * ----------------------------------------------
	 * | 根据云空间的id获取其相应的站点信息
	 * | @时间: 2016年11月5日 下午2:43:48
	 * | @author: duanbin
	 * | @param: variable
	 * | @return: type
	 * ----------------------------------------------
	 */
	public function getSiteInfoByCloudspaceId($id){
		$where['cloudspace_id'] = [ 'eq', $id ];
		return M('cloudspace_site')->where($where)->select();
	}
