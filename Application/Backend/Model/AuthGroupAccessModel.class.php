 ...
 * 权限规则model
 */
class AuthGroupAccessModel extends BackendModel{
	
	protected $trueTableName = 'agent_auth_group_access';
	...
	public function getAllData(){
		$data=$this
			...
			->alias('aga')
			->join(C('AUTH_CONFIG.AUTH_USER').' u ON aga.uid=u.id','RIGHT')
			->join(C('AUTH_CONFIG.AUTH_GROUP').' ag ON aga.group_id=ag.id','LEFT')
			->select();
		// 获取第一条数据
		$first=$data[0];
		$first['title']=array();
